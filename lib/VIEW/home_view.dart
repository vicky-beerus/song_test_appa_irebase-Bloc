...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
  create: (context) => SongBloc(songRepository: RepositoryProvider.of<SongRepository>(context))..add(SongLoadingEvent()),
  child: Scaffold(

      body: BlocBuilder<SongBloc, SongState>(
  builder: (context, state) {

    if(state is SongLoding){
      return Container(
      ...
      return StreamBuilder<List<SongModel>>(
        stream: RepositoryProvider.of<SongRepository>(context).steamGetSong(),
        builder: (context, snapshot) {
          ...
          return snapshot.hasData ?Container(
            height: h,
            width: w,
            child:  snapshot.data?.length == 0? Center(child: Text("Add Songs"),): ListView.builder(itemCount: snapshot.data?.length,itemBuilder: (BuildContext context, int index){
